...
\subsubsection*{[3]}
(1) 略．(2) $(0,0)$と$(1,0)$が開集合で分離できない．(3) 易しい．(4) $\mathcal{O}$から定まる位相の開集合系は
\begin{equation}
    \qty{C\subset X\mid (0,0)\in C}\cup \mathcal{P}(\qty{(1,0),(-1,0),(0,1),(0,-1)})
\end{equation}
であることに注意する．$Q\subset X$が可算ならば$Q\subset \qty{(0,0)}$は$Q$を含む閉集合であるから$\overline{Q}\cup Q\cup \qty{(0,0)}$．$X$は非可算集合なので$\overline{Q}=X$とはなり得ない．
