...
\begin{equation}
    \infty=\int_{t\to t_0-0}\int_{0}^{u(t)}\frac{dx}{f(x)}=t_0<\infty 
\end{equation}
となり矛盾．
\begin{equation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
と定める．$T^{(N)}_{\alpha}$はfinite rank (像が有限次元)であるからコンパクト作用素．一方，
\begin{equation}
...
\end{equation}
である．今，$g\in L^2(X)$より$\abs{g}<\infty\ (\text{$\mu$-a.e.})$であり，測度の単調収束定理より(測度有限に注意)
\begin{equation}
    \mu\qty(\bigcap_{k=1}^\infty G_k)=\lim_{k\to\infty}\mu(G_k)
\end{equation}
なので，$\abs{g}^2\1_{G_k}\to 0\ (\mu\text{-a.e.})$である．$\abs{g}^2\1_{G_k}\leq \abs{g}^2\in L^1$よりdominated convergenceが使えて
\begin{equation}
